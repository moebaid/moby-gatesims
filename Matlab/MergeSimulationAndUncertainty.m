...
listDosesquared_images = dir([path, '/output/*-Dose-Squared.raw']);  % dir-command loads all files with ending -Dose-Squared.raw
listofDosesquared_images = {listDosesquared_images.name};
listDose_images = dir([path, '/output/*-Dose.raw']);  % dir-command loads all files with ending -Dose.raw
listofDose_images = {listDose_images.name};
listEdep_images = dir([path, '/output/*-Edep.raw']);  % dir-command loads all files with ending -Dose.raw
...
Dose_per_decay=Dose_weightedaverage/no_events_split;
% Edep_all=Edep_weightedaverage/no_events_split;


% Unit [Gy]
% Dose_total=Dose_per_decay*Total_Acc;
...
% Dose_per_decay is matrix with deposited dose per total number of primaries in
...
stat_uncertainty_voxel = sqrt((1/(totalevents-1))*(Dosesquared_merged/totalevents - (Dose_per_decay).^2));

rel_uncertainty = stat_uncertainty_voxel./Dose_per_decay*100;
rel_uncertainty(isnan(rel_uncertainty)) = 100;

% Save to files

%% Save to interfile format 
name_dose_total=sprintf([path, '/Dose_total-', sourceorgan, '.img']);
fileID = fopen(name_dose_total,'w');
fwrite(fileID,Dose_per_decay,'float','l');
fclose(fileID);
...
name_headerDK = sprintf([path, '/Dose_total-', sourceorgan, '.hdr']);
fileID=fopen(name_headerDK,'w');
fprintf(fileID,new_header);
fclose(fileID);

%% Save to interfile format 
name_rel_uncertainty=sprintf([path, '/Rel_uncertainty-', sourceorgan, '.img']);
fileID = fopen(name_rel_uncertainty,'w');
fwrite(fileID,rel_uncertainty,'float','l');
fclose(fileID);

name_stat_uncertainty=sprintf([path, '/Stat_uncertainty-', sourceorgan, '.img']);
fileID = fopen(name_stat_uncertainty,'w');
fwrite(fileID,stat_uncertainty_voxel,'float','l');
fclose(fileID);
...
%% Save to header textfile
name_header = sprintf([path, '/Rel_uncertainty-', sourceorgan, '.hdr']);
